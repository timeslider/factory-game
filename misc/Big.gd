...
## AA suffixes keps in dictionary to prevent generating each of them again and again
static var suffixes_aa_new: Dictionary[int, String] = {
	0 : "",
	1 : "k",
	2 : "m",
	3 : "b",
...
## AA suffixes keps in dictionary to prevent generating each of them again and again
static var suffixes_aa_new_2: Dictionary[int, String] = {
	0 : "",
	1 : "k",
	2 : "m",
	3 : "b",
...
## Verifies (or converts) an argument into a Big number
static func _type_check_new(n) -> Big:
	if n is Big:
		return n
	return Big.new(n)
...
## [url]=https://en.wikipedia.org/wiki/Normalized_number[/url] a Big number
...
## [url]=https://en.wikipedia.org/wiki/Normalized_number[/url] a Big number
static func normalize_new(big: Big) -> void:
	if big.mantissa == 0.0:
		big.mantissa = 0.0
		big.exponent = 0
	...
	while big.mantissa >= 10.0:
		big.mantissa *= 0.1
		big.exponent += 1
	big.mantissa = snappedf(big.mantissa, MANTISSA_PRECISION)
...
## Returns the absolute value of a number in Big format
static func absolute_new(x) -> Big:
	var result: Big = Big.new(x)
	result.mantissa = abs(result.mantissa)
	return result
...
## Adds two numbers and returns the Big number result [br][br]
static func add_new(x: Big, y: Big) -> Big:
	return Big.new(x.mantissa + (y.mantissa * 10 ** (y.exponent - x.exponent)))
...
static func subtract(x, y) -> Big:
	x = Big._type_check(x)
	y = Big._type_check(y)
	var negated_y: Big = Big.new(-y.mantissa, y.exponent)
	return add(negated_y, x)


## Subtracts two numbers and returns the Big number result
static func subtract_new(x: Big, y: Big) -> Big:
	return add(Big.new(-y.mantissa, y.exponent), x)
...
## Multiplies two numbers and returns the Big number result
static func times_new(x: Big, y: Big) -> Big:
	return Big.new(x.mantissa * y.mantissa, x.exponent + y.exponent)
...
## Divides two numbers and returns the Big number result
static func division_new(x: Big, y: Big) -> Big:
	if abs(y.mantissa) < MANTISSA_PRECISION:
		printerr("Big Error: Divide by zero or less than " + str(MANTISSA_PRECISION))
		return x
...
## Raises a Big number to the nth power and returns the Big number result
static func powers_new(x: Big, y: Big) -> Big:
	#var result: Big = Big.new(x)
	# (m x 10^e)^p = m^p x 10^(e * p)
	#result.mantissa = x.mantissa ** y.mantissa
	var log_x = log_10(x.mantissa) + x.exponent
	var p = y.mantissa * (10 ** y.exponent)
	var z = p * log_x
	var e_r = floor(z)
	var m_r = 10 ** (z - e_r)
	return Big.new(m_r, e_r)


## Raises a Big number to the nth power and returns the Big number result
static func powers_new_2(x: Big, y: Big) -> Big:
	...
	#var e_r = floor(z)
	#var m_r = 10 ** (z - e_r)
	var z = y.mantissa * (10 ** y.exponent) * (log_10(x.mantissa) + x.exponent)
	var e_r = floor(z)
	return Big.new(10 ** (z - e_r), e_r)
...
static func modulo(x, y) -> Big:
	var result: Big = Big.new(x.mantissa, x.exponent)
	y = Big._type_check(y)
...
static func log_10(x) -> float:
	return log(x) * 0.43429448190325182765112891891661


func abs_log_10() -> float:
	return exponent + Big.log_10(abs(mantissa))
...
static func set_thousand_separator(separator: String) -> void:
	options.thousand_separator = separator


## Sets the Decimal Separator option
static func set_decimal_separator(separator: String) -> void:
	options.decimal_separator = separator


## Sets the Suffix Separator option
static func set_suffix_separator(separator: String) -> void:
...
## Converts the Big Number into a string (in Scientific format)
func to_scientific_new(no_decimals_on_small_values = false, force_decimals = false) -> String:
	if exponent < 3:
		var decimal_increments: float = 1 / (10 ** options.scientific_decimals / 10)
		var value: String = str(snappedf(mantissa * 10 ** exponent, decimal_increments))
		var split: PackedStringArray = value.split(".")
...
func to_logarithmic(no_decimals_on_small_values = false) -> String:
	...
	if exponent < 3:
		var value: String = str(snappedf(mantissa * 10 ** exponent, decimal_increments))
		var split: PackedStringArray = value.split(".")
		if no_decimals_on_small_values == true:
			return split[0]
		if split.size() > 1:
			for i in range(options.logarithmic_decimals):
	...
	for i in range(options.logarithmic_decimals):
		if dec.length() < options.logarithmic_decimals:
			dec += "0"
	var formated_exponent: String = format_exponent(exponent)
...
func _latin_prefix(european_system) -> String:
	var ones: int = _latin_power(european_system) % 10
	var tens: int = int(_latin_power(european_system) / floor(10)) % 10
	@warning_ignore("integer_division")
	var hundreds: int = int(_latin_power(european_system) / 100) % 10
	@warning_ignore("integer_division")
	var millias: int = int(_latin_power(european_system) / 1000) % 10
...
func _tillion_or_illion(european_system) -> String:
	if exponent < 6:
		return ""
	var power_kilo: int = _latin_power(european_system) % 1000
...
func to_aa(no_decimals_on_small_values = false, use_thousand_symbol = true, force_decimals=false) -> String:
	...
	return prefix + options.suffix_separator + suffix + " (old)"

# NOTE: This is crap. It's only a little bit faster than the original but only goes up to zz
## Converts the Big Number into a string (in AA format)
func to_aa_new(no_decimals_on_small_values = false, use_thousand_symbol = true, force_decimals=false) -> String:
	var prefix = to_prefix(no_decimals_on_small_values, use_thousand_symbol, force_decimals)
	var suffix: String = ""
	...
	if suffixes_aa_new.has(target):
		return prefix + options.suffix_separator + suffixes_aa_new[target]
	
	if target < 5:
		suffix = suffixes_aa_new[target]
	else:
		var unit_int = target - 5
		@warning_ignore("integer_division")
		suffix = char((unit_int / 26) + 97) + char((unit_int % 26) + 97)
		suffixes_aa_new[target] = suffix
	

	return prefix + options.suffix_separator + suffix + " (new)"


## Converts the Big Number into a string (in AA format)
func to_aa_new_2(no_decimals_on_small_values = false, use_thousand_symbol = true, force_decimals=false) -> String:
	var prefix = to_prefix(no_decimals_on_small_values, use_thousand_symbol, force_decimals)
	var suffix: String = ""
	...
	if suffixes_aa_new_2.has(target):
		suffix = suffixes_aa_new_2[target]
	else:
		var offset: int = target + 22
		var base: int = 26
		while offset > 0:
			...
			suffix = alphabet_aa[digit] + suffix
			offset /= base
		suffixes_aa_new_2[target] = suffix

	if not use_thousand_symbol and target == 1:
		suffix = ""


	return prefix + options.suffix_separator + suffix + " (new 2)"
